  ...
  return (
    <section className={`topbar ${isActive ? "active" : ""}`}>
      ...
      <div className="topbar-info">
        {loggedInUser && loggedInUser.averageLearningTimePerDayMin &&
          <div className="learning-info">
            <FaClock size="25px" color={getLearningTimeColor(loggedInUser.averageLearningTimePerDayMin)} />
            <span>{loggedInUser.averageLearningTimePerDayMin} min</span>
          </div>}
        {loggedInUser && loggedInUser.learnignStreakInDays &&
          <div className="streak-info">
            <FaFire size="25px" color={getStreakColor(loggedInUser.learnignStreakInDays)} />
            <span>{loggedInUser.learnignStreakInDays}</span>
          </div>}
      </div>
